<template>
  <div class="newslistdiv">
    <div>
      <AddNews class="a1" v-on:click="loadNews()"/>
        ...
        <div class="news-id-title">
          <ul class="list">
            <li>
              ...
              <i class="el-icon-edit"  style="cursor:pointer;" v-on:click="editNews(item)" ref="editnews"/>
              <!--<div class="xian"/>-->
              <div class="clear"></div>
              <!--<p><img :src="item.newsimg1" width="300" height="120"  alt="" /></p>-->
              <!--<button class="del" v-on:click="deleteNews(item.newsid)">删除</button>
              <button class="edit" @click="editNews(item)" ref="editnews">修改/详情</button>-->
            </li>
          </ul>
        </div>
      </el-tooltip>
      <edit-news @addnews="loadNews()"  ref="test111"/>
      <!--<div @onSubmit="loadNews()" ref="addnews"/>-->
    </el-row>
    <el-row>
      <el-pagination
...
<style scoped>
  ...
  .el-icon-edit :hover{cursor: hand}
